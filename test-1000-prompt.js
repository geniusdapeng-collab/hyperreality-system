...
async function test() {
  const engine = new LLMEngine({model: 'kimi-k2p6', timeoutMs: 180000});
...
## 用户痛点（必须涉及）
1. 写Prompt像写天书，3个月学不明白，试了100次还是垃圾
2. 外包做视频10万预算，效果还差，改到第20版还是不满意
3. 等3个月才出片，商机都错过了，竞品早就跑远了
4. 渲染出来的角色手指残缺、脸是歪的，根本没法用
...
- 只输出JSON，不要其他文字`;
  ...
  console.log('[TEST] 1000字符prompt测试...');
