...
视频版全面接入底座 v3.0 通用模型路由：**文本 LLM 是加工费、视频渲染是原材料费——两类成本分池分管，渲染前先过质量门和预算闸，不烧冤枉钱。**

- **vendor 引擎全量收口**：SuperMickey 预生产 14 个 LLM agent（情报/创意/PRD/剧本/分镜/提示词融合/质检修复）全部经 `routeSmart` 路由（`preproduction` 场景 × 降级链 × 真实计量 × `model.call` 事件留痕），fastModel 快速调用自动强制 L1；vendor 目录保持只读
- **多模态生成池**：渲染走异步任务制（submit→task_id→poll→回填），**Seedance → 可灵 → 即梦三供应商降级链**（gen.degraded 全程留痕）；轮询回填自动推进 `render_jobs` 状态机（`video.render.poll`）
- **渲染额度台账**：套餐秒数配额（智享按量实扣 / 标准 100s / 智能 500s 每月），用量 = `render.submit` 事件投影不重算；G8 围栏之后再加**预算闸**——超支未确认即熔断并指引加油包
- **视频场景路由表**（`bundles/ai-video/model-policy.yml`）：评论五分类 L1 谷时批量；预生产/导演评审/提示词融合 L2 谷时排产；账号体检报告 L3 + `bill_to: platform`（售前获客平台成本）
- **谷时排产**：夜班监控与批量内容生产天然落在 22:00–08:00 谷时窗口，费率 ×0.2
...
## 四·五、先体检，再托管：账号快扫，当场出报告

把账号交给系统之前，你可能只有一个问题：**它靠不靠谱？** 答案是：先别托管，先体检。

> **客户旅程（零风险先行）**：新客户默认从「质检模式（Audit-Only）」起步——接上你的社媒账号（抖音/小红书/B站/YouTube 等）后系统**只读扫描、不发一条内容、不回一条评论、不动一分钱投放**，四线巡检（账号健康/内容健康/评论与口碑/转化健康），产出《账号体检报告》（一账号一份+矩阵总览+整改建议，按预估涨粉/线索挽回排序）。你在平台侧人工处置、系统跟踪闭环；看到漏掉的粉和线索之后，再一键切换影子模式（3 天只看不做）→ 正式托管。

**体检分两档**：

- ⚡ **账号快扫**（fast-scan 技能）——授权后 **15–30 分钟**基于 90 天历史数据快照完成四线深度扫描，**当场出《账号快速体检报告》**（售前验货、接入基线）：权重信号与限流风险、断更节律与低效选题、存量负面与高意向咨询遗漏、主页转化组件缺失与线索流失点，Top10 行动清单按预估收益排序、估算口径全标注。
- 🔍 **持续观察**（账号经营管线）——1–2 周观察期，捕捉只有时间能回答的问题：流量曲线实测、评论响应时长、发布窗口效果、趋势与峰值行为。

**围栏纪律**：体检期一切对外写操作物理 block（`fences/patches/audit-only-patch.yml`，发布/回评论/私信/改简介/投放全阻断），上岗员工全部是只读/分析岗——**系统不动一根手指**。

## 五、夜班员工：你睡了，店还开着

内容生意的残酷在于：**流量不睡觉**。

> **夜班的定义（社媒营销版·流量节律型）**：夜班锚的从来不是"AI 的工作时间"——AI 本来就 24 小时全勤——而是**"人的离线时间"**。它是**人类免打扰时段的系统运行模式**：① 触达上免打扰（非 P0 不叫人，聚合为早八点战报）；② 权限上自降级（夜间只做巡检/采集/候选回复等防守性动作，对外发布一律留痕可审）；③ 社媒的高峰窗口是**流量晚高峰与跨时区**——按各平台流量曲线排期发布、评论区 30 分钟级巡检承接；④ 成本上是洼地——**视频渲染/素材批量生产放在谷时算力窗口**（费率 ≤20%），夜间生产、高峰发布。一句话：**夜班服务的不是"夜晚"，是"人的离线"——流量不睡觉，但你可以睡。**
